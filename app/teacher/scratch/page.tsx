// texagonui/app/teacher/scratch/page.tsx
"use client";
...
export default function TeacherScratchPage() {
    ...
    // Bumping `reloadKey` remounts the iframe — cleanest way to fully reset it.
    const [reloadKey, setReloadKey] = useState(0);

    // Safety-net: hide spinner after 25 s even if onLoad misfires.
    useEffect(() => {
        const t = window.setTimeout(() => setIsLoading(false), 25000);
        return () => window.clearTimeout(t);
    }, [reloadKey]);

    // Toggle teacher layout header visibility for focus mode.
    useEffect(() => {
        const layoutHeader = document.getElementById("teacher-layout-header");
        if (layoutHeader) {
                ...
                <div className="flex items-center justify-between gap-2 border-b border-[#EF7B553a] bg-white px-3 py-2">
                    <div className="flex min-w-0 items-center gap-2">
                        ...
                        <span className="hidden items-center gap-1 text-[11px] text-slate-500 sm:inline-flex">
                            ...
                            Demo &amp; prepare projects — use &quot;Save to your computer&quot; in the File
