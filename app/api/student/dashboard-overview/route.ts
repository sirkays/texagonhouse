...
export async function GET(request: Request) {
  console.log("[Route] Received GET request to /api/dashboard")
  const session = await getServerSession(authOptions)
  console.log("[Route] Session data:", { sessionToken: session?.user?.sessionToken })
  ...
  try {
    console.log("[Route] Fetching data from", `${BASE_URL}/accounts/api/dashboard/overview/`)
