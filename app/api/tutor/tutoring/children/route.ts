// app/api/tutor/tutoring/children/route.ts
...
export async function GET(req: Request) {
  noStore();
  try {
    const result = await djangoFetch("/api/tutor/tutoring/children/", {
      method: "GET",
    });
    ...
    if (!result.response.ok) {
      ...
      return attachSetCookie(
        NextResponse.json({ error: "Failed to fetch children" }, { status: result.response.status }),
        result.setCookie
      );
    }
