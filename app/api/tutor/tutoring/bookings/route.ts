// app/api/tutor/tutoring/bookings/route.ts
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const endpoint = `/api/tutor/tutoring/bookings/${queryString ? `?${queryString}` : ""}`;
    ...
    if (!result.response.ok) {
      ...
      return attachSetCookie(
        NextResponse.json({ error: "Failed to fetch parent bookings" }, { status: result.response.status }),
        result.setCookie
      );
    }
