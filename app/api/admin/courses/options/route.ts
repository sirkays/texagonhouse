...
//   console.log(
//     "[Courses Route] Received GET request to /api/admin/courses/options"
//   );
...
//       ? `${BASE_URL}/api/admin/courses/options/?${queryString}`
//       : `${BASE_URL}/api/admin/courses/options/`;
...
//         {error: data.detail || "Failed to fetch course options"},
//         {status: res.status}
//       );
//     }
...
//   } catch (error) {
//     console.error("[Courses Route] Error fetching course options:", error);
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const url = queryString
      ? `${BASE_URL}/api/admin/courses/options/?${queryString}`
      : `${BASE_URL}/api/admin/courses/options/?org_id=1`;
    console.log("[Courses Route] Fetching data from", url);
    const res = await fetch(url, {
      headers: {
        Authorization: `Api-Key ${API_KEY}`,
