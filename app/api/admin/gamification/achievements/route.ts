...
export async function GET(_req: Request) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // if auth is missing/expired, proxy returns 401 from backend
      let errorResp: any = { error: "Failed to fetch achievements" };
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      let errorResp: any = { error: "Failed to create achievement" };
