...
const fetchSubmissionDetail = async (id: number) => {
  const r = await fetch(`/api/code-ide/submissions/${id}`, { cache: "no-store" });
  ...
  return (
    <Card className="flex flex-col w-full submission-tab">
      ...
      <CardContent className="flex-1 flex flex-col gap-4">
        ...
        <Button onClick={handleSubmitClick} disabled={!selectedLesson || isSubmitting}>
            ...
            <CardContent className="space-y-3">
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="col-span-2">
                  <span className="font-medium">Title:</span> {viewing.title || "N/A"}
                </div>
