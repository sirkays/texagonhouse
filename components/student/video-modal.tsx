...
//     case "TOGGLE_QUALITY_SUBMENU":
//       return {...state, showQualitySubmenu: !state.showQualitySubmenu};
//     default:
//       return state;
//   }
// };
...
//                 onClose();
//               }}
//               className={`w-full px-4 py-2 text-left text-sm hover:bg-white/10 transition-colors ${quality === q ? "bg-red-600" : ""}`}>
//               {q}
//             </button>
//           ))}
//         </div>
...
//               onSetQuality={(q) => dispatch({type: "SET_QUALITY", payload: q})}
//               showQualitySubmenu={state.showQualitySubmenu}
//               onToggleQualitySubmenu={() =>
//                 dispatch({type: "TOGGLE_QUALITY_SUBMENU"})
//               }
//               onClose={() => dispatch({type: "TOGGLE_SETTINGS"})}
//             />
//           )}
...
//     if (videoRef.current && videoUrl) {
//       try {
//         const savedStr = localStorage.getItem(`video-progress-${videoUrl}`);
//         if (savedStr) {
//           const saved = JSON.parse(savedStr);
//           const time = saved.currentTime || 0;
//           if (time > 0 && time < (videoRef.current.duration || Infinity)) {
//             videoRef.current.currentTime = time;
//             dispatch({type: "SET_TIME", payload: time});
//           }
...
//           videoRef.current.volume = vol;
//           dispatch({type: "SET_VOLUME", payload: vol});
//           const muted = saved.isMuted || false;
//           videoRef.current.muted = muted;
//           dispatch({type: "SET_MUTED", payload: muted});
//           const speed = saved.playbackSpeed || 1;
//           videoRef.current.playbackRate = speed;
//           dispatch({type: "SET_SPEED", payload: speed});
//         }
//       } catch {}
...
//           const currentIndex2 = speeds2.indexOf(state.playbackSpeed);
//           if (currentIndex2 < speeds2.length - 1) {
//             const newSpeed = speeds2[currentIndex2 + 1];
//             dispatch({type: "SET_SPEED", payload: newSpeed});
//             if (videoRef.current) videoRef.current.playbackRate = newSpeed;
//           }
//           break;
...
//   // Sync state to video properties
//   useEffect(() => {
//     if (videoRef.current) {
//       videoRef.current.muted = state.isMuted;
//     }
...
// // ─────────────────────────────────────────────────────────────────────────────
// // EXPORTED LISTS – "List all video playback speed" + "List all video quality"
// // Import these anywhere in your app:
// // import { playbackSpeeds, videoQualities } from "@/components/VideoModal";
// export const playbackSpeeds = [0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2, 3, 4];
...
//   // ─────────────────────────────────────────────────────────────────────────────
//   // PERSISTENT PROGRESS (seamless resume after close/refresh/browser restart)
//   // ─────────────────────────────────────────────────────────────────────────────
//   const saveProgress = useCallback(() => {
//     if (!videoUrl) return;
...
//       volume: videoRef.current?.volume ?? state.volume,
//       isMuted: videoRef.current?.muted ?? state.isMuted,
//       playbackSpeed: videoRef.current?.playbackRate ?? state.playbackSpeed,
//       quality: state.quality,
//     };
...
//       // Volume, mute, speed, quality
//       const vol = Number(saved.volume ?? 1);
//       videoRef.current.volume = vol;
//       dispatch({type: "SET_VOLUME", payload: vol});

//       const muted = Boolean(saved.isMuted);
//       videoRef.current.muted = muted;
//       dispatch({type: "SET_MUTED", payload: muted});

//       const speed = Number(saved.playbackSpeed ?? 1);
//       videoRef.current.playbackRate = speed;
//       dispatch({type: "SET_SPEED", payload: speed});

//       dispatch({type: "SET_QUALITY", payload: saved.quality ?? "Auto"});
//     } catch (e) {
//       console.warn("Failed to restore video progress", e);
//     }
//   }, [videoUrl]);

//   // Auto-save every 5 seconds while open
//   useEffect(() => {
//     if (!isOpen || !videoUrl) return;
//     const interval = setInterval(saveProgress, 5000);
//     return () => clearInterval(interval);
//   }, [isOpen, videoUrl, saveProgress]);

//   // Save on modal close / page unload
//   useEffect(() => {
//     if (!isOpen) {
//       saveProgress();
//       return;
...
//     if (videoRef.current) {
//       dispatch({type: "SET_DURATION", payload: videoRef.current.duration});
//       restoreProgress(); // ← This is where the magic happens
//     }
//   }, [restoreProgress]);
...
//       videoRef.current.playbackRate = state.playbackSpeed;
//     }
//   }, [isOpen, state.isMuted, state.volume, state.playbackSpeed, saveProgress]);

//   useEffect(() => {
//     if (videoRef.current) videoRef.current.muted = state.isMuted;
//   }, [state.isMuted]);
...
//           dispatch({
//             type: "SET_VOLUME",
//             payload: Math.min(1, state.volume + 0.1),
//           });
//           break;
//         case "ArrowDown":
//           e.preventDefault();
//           dispatch({
//             type: "SET_VOLUME",
//             payload: Math.max(0, state.volume - 0.1),
//           });
//           break;
//         case "KeyM":
//           e.preventDefault();
...
//   }, [isOpen, state.volume, state.duration, toggleFullscreen]);

//   // Double click for seek / fullscreen
//   const handleVideoClick = useCallback(
//     (e: React.MouseEvent<HTMLVideoElement>) => {
//       e.stopPropagation();
//       const now = Date.now();
...
// ─────────────────────────────────────────────────────────────────────────────
// EXPORTED LISTS – Video playback speeds and quality options
// Import these anywhere in your app:
// import { playbackSpeeds, videoQualities } from "@/components/video-modal";
// ─────────────────────────────────────────────────────────────────────────────

export const playbackSpeeds = [
  0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2, 3, 4,
] as const;
export const videoQualities = [
  "Auto",
  "144p",
...
type UIState = {
  ...
  readonly showVolumeSlider: boolean;
  readonly showPlaybackSpeedSubmenu: boolean;
  readonly theaterMode: boolean;
  readonly isFullscreen: boolean;
  readonly showCenterIcon: "play" | "pause" | null;
  readonly hoverTime: number;
  readonly progressHover: boolean;
  readonly quality: VideoQuality;
  readonly showQualitySubmenu: boolean;
};

type InteractionState = {
  readonly controlsTimeout: NodeJS.Timeout | null;
  readonly animationFrame: number | null;
  readonly lastActivity: number;
  readonly doubleClickTimer: NodeJS.Timeout | null;
  readonly lastClick: {
...
const INITIAL_PLAYBACK_SPEED = 1;
const INITIAL_QUALITY: VideoQuality = "Auto";
const VOLUME_INCREMENT = 0.1;
const SEEK_BACKWARD_TIME = 10;
const SEEK_FORWARD_TIME = 10;
const SEEK_ARROW_TIME = 5;
const DOUBLE_CLICK_THRESHOLD = 300;
const CENTER_ICON_DURATION = 500;
...
// Utility function for time formatting (used by ProgressBar and Controls)
