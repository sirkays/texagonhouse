...
// texagon_academy\texagonui\components\admin\modals\invoice-details-modal.tsx
"use client";
...
import { Download, DollarSign, Calendar, Loader2 } from "lucide-react";
...
export function InvoiceDetailsModal({
  ...
  const handleDownload = async () => {
    try {
      const id = invoiceDetails?.id || invoice?.id;
      if (!id) return;
      ...
      const filename =
        (invoiceDetails?.number || invoice?.number || `invoice-${id}`) + ".pdf";
