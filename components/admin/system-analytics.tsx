  ...
  return (
    <div className="space-y-4 p-3 xs:p-4 sm:p-6 max-w-full mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 xs:gap-4">
        <div>
          <h1 className="text-xl xs:text-2xl sm:text-3xl font-bold">System Analytics</h1>
          <p className="text-muted-foreground text-xs xs:text-sm sm:text-base">Comprehensive platform performance and usage analytics</p>
        </div>
        <div className="flex flex-col xs:flex-row gap-2 xs:gap-3">
          <Button variant="outline" className="flex items-center gap-2 bg-transparent text-xs xs:text-sm sm:text-base">
        ...
        {platformMetrics.map((metric, index) => (
          <Card key={index} className="hover:shadow-md transition-shadow">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-xs xs:text-sm font-medium">{metric.title}</CardTitle>
              <div className={`p-1.5 xs:p-2 rounded-full ${metric.bgColor}`}>
                <metric.icon className={`h-3 w-3 xs:h-4 xs:w-4 ${metric.color}`} />
              </div>
            </CardHeader>
            <CardContent>
              <div className="text-lg xs:text-xl sm:text-2xl font-bold">{metric.value}</div>
      ...
      <div className="grid gap-4 xs:gap-6 grid-cols-1 md:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base xs:text-lg sm:text-xl">User Engagement Metrics</CardTitle>
            <CardDescription className="text-xs xs:text-sm sm:text-base">Platform usage and activity levels</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 xs:space-y-4">
            {engagementData.map((item, index) => (
              <div key={index} className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-xs xs:text-sm font-medium">{item.metric}</span>
                  <div className="flex items-center gap-1 xs:gap-2">
                    <span className="text-xs xs:text-sm sm:text-base font-bold">{item.value}</span>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription className="text-xs xs:text-sm sm:text-base">Income sources and distribution</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 xs:space-y-4">
            {revenueBreakdown.map((item, index) => (
              <div key={index} className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-xs xs:text-sm font-medium">{item.source}</span>
                  <span className="text-xs xs:text-sm sm:text-base font-bold text-green-600">{item.amount}</span>
                </div>
                <div className="flex items-center gap-1 xs:gap-2">
                  <div className="flex-1 bg-gray-200 rounded-full h-1.5 xs:h-2">
                    <div className={`h-1.5 xs:h-2 rounded-full ${item.color}`} style={{ width: `${item.percentage}%` }}></div>
                  </div>
                  <span className="text-[0.65rem] xs:text-xs sm:text-sm text-muted-foreground">{item.percentage}%</span>
                </div>
              </div>
            ))}
          </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base xs:text-lg sm:text-xl">Content Performance Metrics</CardTitle>
          <CardDescription className="text-xs xs:text-sm sm:text-base">Usage statistics for different content types</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-4 grid-cols-1 xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4">
            {contentMetrics.map((content, index) => (
              <div key={index} className="p-3 xs:p-4 border rounded-lg space-y-2 xs:space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="font-medium text-xs xs:text-sm sm:text-base">{content.type}</h4>
                  <Badge variant="outline" className="text-[0.65rem] xs:text-xs sm:text-sm">{content.count}</Badge>
                </div>
                <div className="space-y-1 text-[0.65rem] xs:text-xs sm:text-sm text-muted-foreground">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base xs:text-lg sm:text-xl">Top Performing Content</CardTitle>
          <CardDescription className="text-xs xs:text-sm sm:text-base">Most successful courses and materials</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            {topPerformingContent.map((content, index) => (
              ...
              >
                <div className="space-y-1 mb-2 sm:mb-0">
                  <h4 className="font-medium text-xs xs:text-sm sm:text-base">{content.title}</h4>
                ...
                <div className="text-right">
                  <div className="font-medium text-green-600 text-xs xs:text-sm sm:text-base">{content.revenue}</div>
                  <div className="text-[0.65rem] xs:text-xs sm:text-sm text-muted-foreground">Revenue generated</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid gap-4 xs:gap-6 grid-cols-1 md:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription className="text-xs xs:text-sm sm:text-base">Platform performance indicators</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 xs:space-y-4">
            {systemHealth.map((health, index) => (
              <div key={index} className="flex justify-between items-center">
                <span className="text-xs xs:text-sm font-medium">{health.metric}</span>
                <div className="flex items-center gap-1 xs:gap-2">
                  <span className={`text-xs xs:text-sm sm:text-base font-bold ${health.color}`}>{health.value}</span>
                  <Badge
                    variant={health.status === "excellent" ? "default" : "secondary"}
                    className={`${health.status === "excellent" ? "bg-green-100 text-green-800" : ""} text-[0.65rem] xs:text-xs sm:text-sm`}
                  >
                    {health.status}
                  </Badge>
                </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base xs:text-lg sm:text-xl">Recent Activity Summary</CardTitle>
            <CardDescription className="text-xs xs:text-sm sm:text-base">Last 24 hours platform activity</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 xs:space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-xs xs:text-sm">New User Registrations</span>
              <span className="text-base xs:text-lg sm:text-xl font-bold text-blue-600">234</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-xs xs:text-sm">Lessons Completed</span>
              <span className="text-base xs:text-lg sm:text-xl font-bold text-green-600">1,456</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-xs xs:text-sm">Tests Taken</span>
              <span className="text-base xs:text-lg sm:text-xl font-bold text-purple-600">567</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-xs xs:text-sm">Revenue Generated</span>
              <span className="text-base xs:text-lg sm:text-xl font-bold text-orange-600">₦89,000</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-xs xs:text-sm">Support Tickets</span>
              <span className="text-base xs:text-lg sm:text-xl font-bold text-red-600">12</span>
            </div>
          </CardContent>
        </Card>
      </div>
