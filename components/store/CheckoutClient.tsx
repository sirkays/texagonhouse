...
export default function CheckoutPage() {
  ...
  const resolveBuyNowPayload = () => {
    ...
    const lsPid =
      (typeof window !== "undefined" && localStorage.getItem(LS_BUYNOW_PID)) ||
      "";
    const lsQty = safeNum(
      (typeof window !== "undefined" && localStorage.getItem(LS_BUYNOW_QTY)) ||
        "1",
      1
    );
  ...
  // ✅ what UI should show
  const subtotal = isBuyNowCheckout
    ? buyNowTotals?.subtotal || 0
    : cartSubtotal;
  const shipping = isBuyNowCheckout
    ? buyNowTotals?.shipping || 0
    : cartShipping;
  const tax = isBuyNowCheckout ? buyNowTotals?.tax || 0 : cartTax;
  const total = isBuyNowCheckout ? buyNowTotals?.total || 0 : cartTotal;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      }/store/checkout?${currentUrl.searchParams.toString()}`;
